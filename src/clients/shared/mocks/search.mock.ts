...
export const getMockSearchResponse = (query: string): SearchResponse => {
  // 간단한 필터링 로직 (실제로는 백엔드에서 처리)
  const filtered = mockResumeResults.filter(
    (result) =>
      result.skills.some((skill) =>
        skill.toLowerCase().includes(query.toLowerCase())
      ) ||
      result.position.toLowerCase().includes(query.toLowerCase()) ||
      result.experienceDetail.toLowerCase().includes(query.toLowerCase())
  );

  return {
    results: filtered.length > 0 ? filtered : mockResumeResults,
    total: filtered.length > 0 ? filtered.length : mockResumeResults.length,
  };
};
